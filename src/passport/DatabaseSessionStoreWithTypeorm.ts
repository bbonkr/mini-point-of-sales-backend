...
export default class DatabaseSessionStore extends expressSession.Store {
  private readonly defaultExpriation =
    1000 * 60 * 60 * 24 * 120; /** 120 days */
  private options: DatabaseSessionStoreOptions;
  private clearExpiredSessionsInterval!: number;
  ...
  constructor(config: DatabaseSessionStoreOptions) {
    ...
    const options = {
      expiration: config.expiration || this.defaultExpriation,
  ...
  public set = (
    ...
    const expireMiliseconds = now.setMilliseconds(
      now.getMilliseconds() +
        (this.options.expiration || this.defaultExpriation)
    );
